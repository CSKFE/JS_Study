...
`옵셔널 체이닝 연산자 ?.`는 객체를 가리키기를 기대하는 변수가 `null` 또는 `undefined`가 아닌지 확인하고 프로퍼티를 참조할 때 유용하다. `옵셔널 체이닝 연산자가 ?.`가 도입되기 이전에는 `논리 연산자 &&`를 사용한 `단축 평가`를 통해 변수가 `null` 또는 `undefined`인지 확인했다.
